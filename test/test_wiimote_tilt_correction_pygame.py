...
def test_wiimote():
    ...
    try:
        ...
        while not is_exit:
            ...
            # alcular el ángulo de rotación
            angle = math.atan2(ir_dots[1][1] - ir_dots[0][1], ir_dots[1][0] - ir_dots[0][0]) * -1

            # Calcular el punto medio
            p_center = [(ir_dots[1][0] + ir_dots[0][0]) / 2, (ir_dots[1][1] + ir_dots[0][1]) / 2]

            # Trasladamos a la esquina superior para rotar en 0,0
            p_center[0] = p_center[0] - 0.5
            p_center[1] = p_center[1] - 0.5

            # rotación inversa
            rotx = math.cos(angle) * p_center[0] - math.sin(angle) * p_center[1];
            roty = math.sin(angle) * p_center[0] + math.cos(angle) * p_center[1];

            # volvemos al punto original
            p_center[0] = 0.5 + rotx
            p_center[1] = 0.5 + roty
            ...
            pygame.draw.circle(surface, [0xe3, 0xc8, 0xe4], p_center, ratio, width=2)
